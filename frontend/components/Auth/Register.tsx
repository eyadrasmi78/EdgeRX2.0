...
import { Activity, ArrowLeft, Upload, Calendar, Globe, Building } from 'lucide-react';
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 py-12 px-4 sm:px-6 lg:px-8" dir={dir}>
      <div className="max-w-3xl w-full space-y-8 bg-white p-8 rounded-xl shadow-lg border border-gray-200">
        <div className="flex items-center justify-between">
           ...
           <div className="flex items-center gap-3"><img src="/logo.png" alt="EdgeRx" className="h-9 w-auto" /><h2 className="text-2xl font-bold text-gray-900">{t('register_company')}</h2></div>
        </div>

        {isSuccess ? (
          <div className="text-center py-12"><div className="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-green-100 mb-4"><Activity className="h-6 w-6 text-green-600" /></div><h3 className="text-lg leading-6 font-medium text-gray-900">{t('registration_successful')}</h3><p className="mt-2 text-sm text-gray-500">{message}</p><p className="mt-4 text-sm text-teal-600">{t('redirecting')}</p></div>
        ) : (
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className={`border rounded-lg p-3 cursor-pointer text-center transition-all ${formData.role === UserRole.CUSTOMER ? 'border-teal-500 bg-teal-50 ring-1 ring-teal-500' : 'border-gray-200 hover:border-gray-300'}`} onClick={() => setFormData({...formData, role: UserRole.CUSTOMER})}><div className="mx-auto w-8 h-8 rounded-full bg-teal-100 flex items-center justify-center mb-2"><Building className="h-4 w-4 text-teal-700" /></div><h3 className="font-semibold text-gray-900 text-sm">{t('role_customer')}</h3></div>
